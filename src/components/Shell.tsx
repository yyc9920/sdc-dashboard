...
export default function VariantShell({ variant, children }: { variant: 'a'|'b'|'c'|'d'|'e'; children: ReactNode }) {
  ...
  return (
    <div className={`v-${variant} h-screen w-screen grid`} style={{ gridTemplateRows: '52px 1fr 68px' }}>
      <header className="flex items-center gap-4 px-5 border-b border-black/5 bg-white/60 backdrop-blur-sm"
        style={{ backgroundColor: variant === 'b' ? 'rgba(10,12,18,.6)' : undefined, color: variant === 'b' ? '#e5e7eb' : undefined, borderColor: variant === 'b' ? 'rgba(255,255,255,.08)' : undefined }}>
        <div className="flex items-center gap-2">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[10px] opacity-60">초등부 세미나 · v2 interactive</div>
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <select
            ...
            className="text-xs rounded-lg border border-black/10 px-2 py-1 bg-white/80 text-slate-800">
            {[...STUDENTS].sort((a,b)=>a.name.localeCompare(b.name,'ko')).map(s => (
              <option key={s.id} value={s.id}>{s.name} · {s.grade}학년</option>
            ))}
          </select>
          {isDetail ? (
            <Link to={`/${variant}`} className="text-xs px-2 py-1 rounded-lg bg-black/5 hover:bg-black/10">← 대시보드</Link>
          ) : (
            <Link to={`/${variant}/student/${selectedId}`} className="text-xs px-2 py-1 rounded-lg bg-black/5 hover:bg-black/10">상세 →</Link>
          )}
        </div>
      </header>
